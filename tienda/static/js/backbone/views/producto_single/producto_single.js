Loviz.Views.Producto_single = Backbone.View.extend({
	...
	modificar:function () {
		var titulo = this.model.toJSON().full_name+' || LovizDelCarpio.com'
		var descripcion = this.model.toJSON().descripcion
		window.views.head.modificar_titulo(titulo)
		window.views.head.modificar_descripcion(descripcion)
	...
	crear_relacionados:function () {
		...
		relacionados.fetch().done(function () {
			relacionados = relacionados.shuffle();
			relacionados.forEach(self.addRelacionados,self);
		});
	},
